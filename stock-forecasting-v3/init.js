...
$('#suggestbutton').click(function(){
  ...
  $('#smooth').val(0.5)
  $('#sigma').val(0.7)
  $('#learningrate_agent').val(0.3)
  $('#populationsize').val(34)
  $('#sizelayer_agent').val(176)
  $('#threshold').val(0.9)
  $('#skip').val(1)
  $('#epoch_agent').val(100)
})
...
$('#trainbutton').click(function(){
  ...
  setTimeout(function(){
    ...
    async_training_loop(function() {
      ...
      JSON.stringify({'close': predicted_val,
      'date': new_date,
      'window_size': parseInt($('#history').val()),
      'population_size': parseInt($('#populationsize').val()),
      'layer_size': parseInt($('#sizelayer_agent').val()),
      'sigma': parseFloat($('#sigma').val()),
      'learning_rate': parseFloat($('#learningrate_agent').val()),
      'money': parseFloat($('#initialmoney').val()),
      'max_buy': parseInt($('#maxbuy').val()),
      'max_sell': parseInt($('#maxsell').val()),
      'skip': parseInt($('#skip').val()),
      'threshold': parseFloat($('#threshold').val()),
      'epoch': parseInt($('#epoch_agent').val())}));
      xmlhttp = new XMLHttpRequest();
      xmlhttp.onreadystatechange = function() {
        if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
          try{
            $(".loadingscreen").css("display", "none");
            return_agent = JSON.parse(this.responseText);
            my_investment = return_agent['agent']
            my_outliers = return_agent['outliers']
            ...
            for (var i = 0; i < my_outliers['date'].length;i++){
              ind = new_date.indexOf(my_outliers['date'][i])
              markpoints.push({name: 'outlier', value: 'outlier', xAxis: ind, yAxis: my_outliers['close'][i],itemStyle:{color:'#FFA500'}})
            }
            option = {
              animation: false,
              color: color_list,
            ...
            $(".loadingscreen").css("display", "none");
            $("#failparagraph").html('Monkey said: '+err);
            $(".loadingscreen-fail").css("display", "block");
            return;
          }
          if(data['error']){
            $(".loadingscreen").css("display", "none");
            $("#failparagraph").html('Monkey said: '+data['error']);
            $(".loadingscreen-fail").css("display", "block");
            return;
          }
          else{
      ...
      xmlhttp.open("POST", "http://huseinhouse.com:8096/stock", true);
